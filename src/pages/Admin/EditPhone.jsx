...
export default function EditPhone() {
  const { id } = useParams();
  const toast = useRef(null); // Toast thông báo
    ...
    const fetchPhoneDetails = async () => {
      try {
        const phoneResponse = await axios.get(
          `https://localhost:7274/api/admin/AdminProduct/${id}`
        );
        const specResponse = await axios.get(
          `https://localhost:7274/api/admin/AdminSpecification/product/${id}`
        );

        setPhoneData(phoneResponse.data);
        setSpecification(specResponse.data);
        setOriginalData({ phone: phoneResponse.data, spec: specResponse.data });
      } catch (error) {
  ...
  const handleReset = () => {
    if (originalData) {
      setPhoneData(originalData.phone);
      setSpecification(originalData.spec);
      toast.current.show({
        severity: "info",
        summary: "Đã đặt lại",
        detail: "Dữ liệu đã được khôi phục!",
        life: 3000,
      });
    }
  };
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto bg-white shadow-lg rounded-xl">
      <Toast ref={toast} /> {/* Thêm Toast để hiển thị thông báo */}
      ...
      <div className="flex flex-col gap-4">
        ...
        <Button
          label="Reset"
          onClick={handleReset}
          className="p-button-secondary bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-3 rounded-lg w-full"
        />
      </div>
    </div>
  );
